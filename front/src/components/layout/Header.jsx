    ...
    <header className='fixed w-full top-0 z-50 bg-white shadow-md p-3 md:px-10'>
      <div className='flex justify-between'>
        <div className='relative flex items-center my-auto cursor-pointer ml-10'>
          <Link href={'/'}>
            <Image src={AirbBnbLogo.src} width={80} height={80} alt='Logo' />
          </Link>
        </div>
        <div className='flex hover:shadow-md  transition-all items-center md:border-2 rounded-full py-2 md:shadow-sm w-1/3 ml-32'>
          <input type="text" className='flex-grow pl-5 bg-transparent outline-none' placeholder='Commencez votre recherche' value={search} onChange={handleSarchInput} />
        ...
        <div className='flex items-center'>
          <Link href={'/auth/login'}>
            <button className='mx-4 cursor-pointer p-1 rounded-full hover:bg-opacity-1 hover:bg-gray-100 h-8 text-ellipsis overflow-hidden'>Mettre mon logement sur Airbnb</button>
          </Link>
          {currentUser?.isAdmin &&
            ...
            {accountMenu &&
              <div className='transition-all bg-white rounded-xl w-64 shadow-sm p-3'>
                ...
                <div className='flex flex-col font-light ml-2'>
                  ...
                  <span className='my-2 border-b-2 pb-4'>Deconnexion</span>
