...
export default function Places() {
  ...
  if (loading) {
    return <p className='text-2xl font-bold text-center mt-30'>Chargement...</p>;
  }
  if (error) {
  ...
  return (
    <div className='my-4 mx-auto' style={{ width: '60%' }}>
      <div className='grid xl:grid-cols-4 md:grid-cols-3 sm:grid-cols-2 mt-44 justify-around cursor-pointer'>
        {list.length > 0 ? list.map((place) =>
          ...
          <div className='absolute text-2xl text-center mt-10' style={{ width: '94%' }}>
            <span className='mx-auto'> Désolé, aucun résultat trouvé !</span>
          </div>}
      </div>
